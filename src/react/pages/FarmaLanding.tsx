  ...
  return (
    <div style={{ minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', background: '#fff', color: '#1a2e35' }}>
      <header style={{ background: `linear-gradient(135deg, ${cor} 0%, ${shade(cor, -20)} 100%)`, color: '#fff', padding: '56px 20px 64px', textAlign: 'center' }}>
        ...
        {zap && <a href={waLink} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, marginTop: 30, background: '#fff', color: cor, fontWeight: 700, padding: '15px 32px', borderRadius: 999, textDecoration: 'none', fontSize: '1.05rem', boxShadow: '0 8px 24px rgba(0,0,0,0.18)' }}>💬 {l.ctaTexto || 'Chamar no WhatsApp'}</a>}
      </header>
      ...
      <section style={{ textAlign: 'center', padding: '20px 20px 60px' }}>
        ...
        {zap && <a href={waLink} style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: cor, color: '#fff', fontWeight: 700, padding: '15px 32px', borderRadius: 999, textDecoration: 'none', fontSize: '1.05rem' }}>💬 {l.ctaTexto || 'Chamar no WhatsApp'}</a>}
      </section>
