...
type Tool = {
  ...
  excludes?: string;   // ferramenta que é desativada ao ativar esta
};

const TOOLS: Tool[] = [
  { key: 'venda_catalogo', name: 'Catálogo', desc: 'Loja própria com carrinho e pagamento (PIX / Mercado Pago). O cliente monta o pedido e você recebe pronto.', icon: 'fa-bag-shopping', color: '#6366f1', open: '/products', excludes: 'vitrine' },
  { key: 'vitrine', name: 'Vitrine', desc: 'Mostruário elegante sem checkout — o cliente vê os produtos e pede direto no WhatsApp.', icon: 'fa-image', color: '#a855f7', open: '/products', excludes: 'venda_catalogo' },
  { key: 'atendimento', name: 'Atendente IA', desc: 'Uma IA que atende, tira dúvidas e qualifica seus clientes no WhatsApp, 24 horas por dia.', icon: 'fa-robot', color: '#22c55e', open: '/leads' },
  { key: 'agendamento', name: 'Agendamento', desc: 'Agenda de serviços com confirmação e lembretes automáticos. Ideal para clínicas, salões e consultorias.', icon: 'fa-calendar-check', color: '#f59e0b', open: '/schedule' },
  { key: 'disparo', name: 'Campanhas', desc: 'Dispare mensagens em massa para toda a sua base no WhatsApp, com inteligência anti-banimento.', icon: 'fa-bullhorn', color: '#ef4444', open: '/campaigns' },
  { key: 'ecommerce', name: 'E-commerce (NuvemShop)', desc: 'Conecte sua loja NuvemShop: automações de WhatsApp, widgets de conversão, analytics e CRM.', icon: 'fa-store', color: '#0ea5e9', soon: true },
];
...
export function Tools() {
  ...
  async function activate(t: Tool) {
    if (t.excludes && modulos?.includes(t.excludes)) {
      const other = TOOLS.find((x) => x.key === t.excludes)?.name || 'a outra';
      const ok = await confirm.warning('Trocar de ferramenta', `Ativar "${t.name}" vai desativar "${other}" (a mesma loja não usa as duas ao mesmo tempo). Deseja continuar?`);
      if (!ok) return;
    }
    setBusy(t.key);
    try {
