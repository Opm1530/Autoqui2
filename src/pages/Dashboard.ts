...
export const Dashboard = async () => {
    ...
    if (user?.role === 'admin') {
        ...
        try {
            ...
            const needsMessages = modulos.includes('atendimento');
            const needsOrders = modulos.includes('venda') || modulos.includes('venda_catalogo');
            const needsCatalog = modulos.includes('venda_catalogo');

            const [messages, orders, products] = await Promise.all([
                needsMessages ? dbService.getAll('messages', { field: 'empresaId', operator: '==', value: user.companyId }) : Promise.resolve([]),
                needsOrders   ? dbService.getAll('pedidos',  { field: 'empresaId', operator: '==', value: user.companyId }) : Promise.resolve([]),
                needsCatalog  ? dbService.getAll('products', { field: 'companyId', operator: '==', value: user.companyId }) : Promise.resolve([]),
            ]);

            if (needsMessages) {
                const userStoreIds = user.storeIds || (user.storeId ? [user.storeId] : []);
                metrics.messages = (messages as any[]).filter((m: any) => {
            ...
            if (needsOrders) {
                ...
                const filteredOrders = (user as any).role === 'owner'
                    ? (orders as any[])
                    : (orders as any[]).filter((o: any) => o.lojaId && userStoreIds.includes(o.lojaId));
                ...
                if (needsCatalog) {
                    ...
                    const lowStockProducts = productsArr.filter((p: any) => p.stock != null && p.stock <= 5 && p.active !== false)
