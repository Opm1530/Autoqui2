...
export const farmaquiApi = {
  ...
  config: (): Promise<{ capturaAtiva: boolean; capturaInstancia: string; recompra: { enabled: boolean; mensagem: string; cicloDiasPadrao: number }; automacoes: { aniversario: { enabled: boolean; mensagem: string }; reativacao: { enabled: boolean; dias: number; mensagem: string }; usoContinuo: { enabled: boolean; mensagem: string } }; fidelidade: { enabled: boolean; meta: number; premio: string; mensagem: string } }> => req('/api/farmaqui/config', 'GET'),
  saveFidelidade: (f: { enabled: boolean; meta: number; premio: string; mensagem: string }) => req('/api/farmaqui/fidelidade', 'POST', f),
  sendFidelidade: (leadId: string): Promise<{ ok: boolean; reason?: string }> => req('/api/farmaqui/fidelidade/enviar', 'POST', { leadId }),
  ...
  manualLead: (nome: string, telefone: string): Promise<{ ok: boolean; nome: string; id: string }> => req('/api/farmaqui/manual-lead', 'POST', { nome, telefone }),
  sendMessage: (leadId: string, text: string) => req('/api/farmaqui/send-message', 'POST', { leadId, text }),
  health: (): Promise<{ instancia: string; connected: boolean; state: string; totalLeads: number; enviadosHoje: number; limiteHoje: number }> => req('/api/farmaqui/health', 'GET'),
