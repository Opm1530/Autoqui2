...
export const farmaquiApi = {
  ...
  config: (): Promise<{ capturaAtiva: boolean; capturaInstancia: string; recompra: { enabled: boolean; mensagem: string; cicloDiasPadrao: number }; automacoes: { aniversario: { enabled: boolean; mensagem: string }; reativacao: { enabled: boolean; dias: number; mensagem: string }; usoContinuo: { enabled: boolean; mensagem: string } } }> => req('/api/farmaqui/config', 'GET'),
