...
export const farmaquiApi = {
  ...
  config: (): Promise<{ capturaAtiva: boolean; capturaInstancia: string; recompra: { enabled: boolean; mensagem: string; cicloDiasPadrao: number }; automacoes: { aniversario: { enabled: boolean; mensagem: string }; reativacao: { enabled: boolean; dias: number; mensagem: string } } }> => req('/api/farmaqui/config', 'GET'),
  saveRecompra: (r: { enabled: boolean; mensagem: string; cicloDiasPadrao: number }) => req('/api/farmaqui/recompra', 'POST', r),
  saveAutomacoes: (a: { aniversario: { enabled: boolean; mensagem: string }; reativacao: { enabled: boolean; dias: number; mensagem: string } }) => req('/api/farmaqui/automacoes', 'POST', a),
